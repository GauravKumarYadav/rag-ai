...
Or run specific test: pytest tests/live/test_endpoints.py::TestHealthAndStatus -v
"""
...
class TestContainerHealth:
    ...
    def test_mysql_accessible(self):
        ...
        try:
            ...
            sock.close()
            assert result == 0, f"MySQL not accessible on port {MYSQL_PORT}"
        except Exception as e:
...
class TestChat:
    ...
    def test_chat_endpoint(self, auth_headers):
        ...
        # Then send a chat message (with longer timeout for LLM)
        response = requests.post(
            f"{BASE_URL}/chat",
            headers=auth_headers,
            json={
